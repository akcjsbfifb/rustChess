...
use rustChess::board::types::{Move, PieceType, Color};
use rustChess::movegen::{generate_moves, is_square_attacked};
...
fn test_is_square_attacked_by_king() {
    let mut board = Board::new();
    for i in 0..BOARD_SIZE { board.squares[i] = OFF_BOARD; } // Limpiar tablero
    board.squares[55] = 0x06; // Rey blanco en e4
    board.squares[64] = 0x86; // Rey negro en d5
    
    assert!(is_square_attacked(&board, 55, Color::Black), "El Rey debería atacar las casillas adyacentes");
    assert!(is_square_attacked(&board, 65, Color::White), "El Rey blanco debería atacar e5");
}
